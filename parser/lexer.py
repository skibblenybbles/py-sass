...
def create_token(t, process_lines):
    """
    Create a Token from the given LexToken and update the lexer's lineno
    and colum values.
    """
    ...
    if process_lines:
        ...
        matches = [m for m in rx_newline.finditer(value)]
        t.lexer.lineno += len(matches)
        
        # reset the column counter?
        if len(matches) > 0:
            m = matches[-1]
            t.lexer.column = len(value) - m.end() + 1
        
        else:
            t.lexer.column += len(value)
...
def t_SYM_CHARSET(t):
    token = create_token(t, False)
    token.parsed = "@charset"
    return t


# SYM_IMPORTANT - !important
# tokens += ("SYM_IMPORTANT",)
# 
# @lex.TOKEN(
#     r'!(' + partials['optspace'] + r'|' + partials['blockcomment'] + ')*' + ''.join([partials[e] for e in "IMPORTANT"])
# )
# def t_SYM_IMPORTANT(t):
#     token = create_token(t, True)
...
def t_simple_token(t):
    t.type = simple_tokens_xlt.get(t.value)
    create_token(t, False)
    return t
...
# the lexer's current column number
lexer.column = 1
